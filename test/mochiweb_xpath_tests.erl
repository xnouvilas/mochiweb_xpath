...
test_definitions() ->
  [
    {?HTML1, [
      ...
      {"replace(/html/body/comment(), 'o', 'ö')", <<" söme cömment "/utf8>>},
      %% replace list
...
all_test_() ->
  ...
  %% (the engine will take care of converting the parameters to the appropiate type (node_set|string|number|boolean)
...
unicode_test() ->
  ...
  ?assertEqual([<<"unicode-class">>], mochiweb_xpath:execute("/html/body/div[@class='юникод']/text()", Doc)),
...
%%     false ->
%%       io:format("*fail*: ~s : ~n* Result:~p Expected:~p~n", [Expr,Result,Expected])
%%     end.
